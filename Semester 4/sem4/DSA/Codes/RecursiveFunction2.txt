...
bool RBinarySearch(int A[], int Key, int low, int high) {
	...
	else if (Key > A[mid]) { return RBinarySearch(A, Key, mid + 1, high); }
	else { return RBinarySearch(A, Key, low, mid-1);
	}
}
...
int* RBubbleSort(int A[], int size) {
...
int main() {
	...
	PrintArray(A, size - 1);
	int* B = RBubbleSort(A, size - 1);
	PrintArray(B, size - 1);
	cout << "Binary Search: " << endl;
	if (RBinarySearch(B,70,0,size-1)) { cout << "Found" << endl; }
	else { cout << "Not-found" << endl; } 

	//cout << "Factorial of 5: " << RFacotial(5)<<endl;
	//cout << " 5 Power 4: " << RPower(5, 4);
}
